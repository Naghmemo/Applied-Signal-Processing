...
for ff = 1:Nfiles
    ...
    MPH = 0.15 * max(x0); % fraction = 0.1 between 0 and 1
    ...
    for i = 1:length(p)
        if p(i) < 2*MPH
            p(i) = 0.5*(p(i-1) + p(i+1));  %replace small R-peaks with the average of next and previous R-peaks 
        end
    end
    ...
    rr = diff(r_indices) % print rr intervals for each iteration
    
    figure(ff+4)
    plot(r_indices(1:end-1)/fs,(1./rr)*fs*60)
    title(['IHR for Data Set Number =',num2str(ff)])
    figure(ff+8)
    histogram(rr) %plot histogram of the RR interval
    title(['Histogram of the RR intervals for data set number =',num2str(ff)])
    % pause
end 
